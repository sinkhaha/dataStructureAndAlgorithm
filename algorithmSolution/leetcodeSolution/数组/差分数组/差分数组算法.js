...
class Difference {
     ...
     * 
     * 例如：
     * 查看getOriginArrFromDiff方法通过diff反推nums的过程，
     * 如果想对区间nums[i..j]的元素全部加 3，
    ...
    increment(i, j, val) {
        this.diff[i] += val;
        // 如果是对nums[i]及以后的整个数组都进行修改，那么就不需要再给diff数组减val了
...
console.log(`原数组是${originNums}`); // [8, 5, 9, 6, 1]

// 给差分值数组第1和第2个元素的值加上2
difference.increment(1, 2, 2);
console.log(difference.getDiff()); // [ 8, -1, 4, -5, -5 ]
const originNums2 = difference.getOriginArrFromDiff(nums);
console.log(`原数组2是${originNums2}`); // [8,7,11,6,1]
